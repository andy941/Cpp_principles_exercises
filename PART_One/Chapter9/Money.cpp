...
Money operator+(Money a, Money b)
{
	...
	int dec = abs(a.Decimal() - b.Decimal());
	long int left;
	
	if (dec != 0) left = pow(10,dec)*a.Amount();
	if (dec == 0) left = a.Amount();

	long int aa =  left + b.Amount();
	dec += min(a.Decimal(), b.Decimal());

	Money result {aa, a.Curr(), dec};
	return  result;
};
